...
describe('Watcher Silence (Sovereign Handoff)', () => {
    ...
    it('should reject external updates when silenced', async () => {
        // Fast forward 3 seconds (normally external)
        vi.advanceTimersByTime(3000);
        expect(await service.isExternalUpdate('test.md', [])).toBe(true);

        // Silence for 1 second
        service.silence('test.md', 1000);

        // Still true for a moment because of how Date.now() works with fake timers? 
        // Let's ensure fake timers are used for Date.now too.
        expect(await service.isExternalUpdate('test.md', [])).toBe(false);
    ...
    it('should respect the custom silence duration', async () => {
        vi.advanceTimersByTime(3000);

        service.silence('test.md', 5000);
        expect(await service.isExternalUpdate('test.md', [])).toBe(false);
