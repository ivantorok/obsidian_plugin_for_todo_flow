...
- [ ] Unit Test: Verify that `ScheduleManager.recalculate()` is NOT called when the `InteractionState` is `active`.
- [ ] E2E Test: Monitor frame rate during a 200ms duration swipe; frame rate must stay above 50fps on throttled hardware.
...
## Context / Constraints
- Involved files: `src/services/ScheduleManager.ts`, `src/views/StackView.svelte`.
