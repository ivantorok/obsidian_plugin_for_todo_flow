...
## 5. Resolution
- **Applied Fix**: Added `e.stopPropagation()` / `e.stopImmediatePropagation()` to `touchstart` and `touchmove` handlers in `StackView.svelte`.
- **Verified**:
    - Unit test in `StackViewMobileGestures.test.ts` confirms propagation is stopped.
    - E2E test `BUG-017_gesture_shadowing.spec.ts` confirms Obsidian sidebar no longer triggers during card swipes.
- **Ship Version**: v1.2.24
- **Date**: 2026-02-09
